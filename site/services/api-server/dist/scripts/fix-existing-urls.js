...
async function main() {
    ...
    const eventsToFix = await database_1.prisma.events.findMany({
        where: {
            OR: [
    ...
    for (const event of eventsToFix) {
        ...
        const newQrCodeUrl = await eventService_1.EventService.generateQRCode(newJoinUrl);
        await database_1.prisma.events.update({
            where: { id: event.id },
            data: {
                join_url: newJoinUrl,
