...
router.put('/profile', authenticateToken, async (req, res, next) => {
    try {
        ...
        const user = await tenantDb.direct.users.update({
            where: { id: userId },
            data: {
                name,
                email,
...
router.put('/notifications', authenticateToken, async (req, res, next) => {
    try {
        const userId = (req as any).user?.id;
...
// Update password
router.put('/security/password', authenticateToken, async (req, res, next) => {
    try {
        const userId = (req as any).user?.id;
...
// Toggle 2FA
router.put('/security/2fa', authenticateToken, async (req, res, next) => {
    try {
        const userId = (req as any).user?.id;
