...
export function FallingPatternBackground({ className, color = "rgba(99,102,241,0.8)", backgroundColor = "#030303" }: { className?: string; color?: string; backgroundColor?: string }) {
  ...
  return (
    <div className={cn("absolute inset-0 overflow-hidden", className)}>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="absolute inset-0">
        <motion.div
          ...
          variants={{
            ...
            animate: { backgroundPosition: [startPositions, endPositions], transition: { duration: 150, ease: "linear", repeat: Infinity } },
          }}
          initial="initial"
          animate="animate"
        />
      </motion.div>
      <div className="absolute inset-0" style={{ backdropFilter: "blur(1em)", backgroundImage: `radial-gradient(circle at 50% 50%, transparent 0, transparent 2px, ${backgroundColor} 2px)`, backgroundSize: "8px 8px" }} />
    </div>
  );
}
