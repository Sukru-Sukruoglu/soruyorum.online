(function ($) {
  ...
  function thmSwiperInit() {
    ...
    if ($(".thm-swiper__slider").length) {
      $(".thm-swiper__slider").each(function () {
        let elm = $(this);
        if (elm.hasClass('swiper-initialized')) return;
        let options = elm.data('swiper-options');
        let thmSwiperSlider = new Swiper(elm[0], options);
  ...
  if ($(".portfolio-two__carousel").length) {
    var initPortfolioTwo = function () {
      if (typeof $.fn.owlCarousel === 'function') {
        ...
        setTimeout(initPortfolioTwo, 100);
      }
    };
    initPortfolioTwo();
  }
  ...
  if ($(".contact-form-validated").length) {
    $(".contact-form-validated").each(function () {
      let self = $(this);
      let resultBox = self.parent().find(".result");
      let submitBtn = self.find('button[type="submit"]');
      ...
      self.on("submit", function (e) {
        ...
        // Get selected subject from nice-select
        var subjectEl = self.find('.selectmenu');
        var subject = "";
        if (subjectEl.length) {
          subject = subjectEl.val() || "";
          if (subject === "Konu Seçiniz") subject = "";
        }
        ...
        if (!name.toString().trim()) {
          resultBox.html('<div class="error" style="color: #ff4444; margin-top: 15px;">Lütfen adınızı girin.</div>');
          return false;
        }
        if (!email.toString().trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.toString().trim())) {
          resultBox.html('<div class="error" style="color: #ff4444; margin-top: 15px;">Lütfen geçerli bir e-posta adresi girin.</div>');
          return false;
        }
        if (!message.toString().trim()) {
          resultBox.html('<div class="error" style="color: #ff4444; margin-top: 15px;">Lütfen mesajınızı yazın.</div>');
          return false;
        }
        ...
        $.ajax({
          ...
          .done(function (data) {
            if (data && data.ok) {
              resultBox.html('<div class="success" style="color: #00c853; margin-top: 15px;">Mesajınız başarıyla gönderildi. En kısa sürede size dönüş yapacağız.</div>');
              self.find('input[type="text"]').val("");
              self.find('input[type="email"]').val("");
              self.find('textarea').val("");
            } else {
              var msg = (data && data.message) || "Bir hata oluştu. Lütfen tekrar deneyin.";
              resultBox.html('<div class="error" style="color: #ff4444; margin-top: 15px;">' + msg + '</div>');
            }
          })
          .fail(function (xhr) {
  ...
  if ($(".newsletter-two__form").length) {
    $(".newsletter-two__form").each(function () {
      var Self = $(this);
      var emailInput = Self.find('input[type="email"]');
      var consentInput = Self.find('input[type="checkbox"]');
      var respBox = Self.find(".result");
      var submitBtn = Self.find('button[type="submit"]');

      if (!respBox.length) {
        respBox = $('<div class="result"></div>');
        Self.append(respBox);
      }
      ...
      Self.on("submit", function (e) {
        ...
        var consent = consentInput.is(":checked");

        respBox.empty();

        if (!consent) {
          respBox.append(
            '<div class="error">Lütfen gizlilik politikasını kabul edin.</div>'
          );
          return false;
        }
        ...
        if (!email) {
          respBox.append(
            '<div class="error">Lütfen e-posta adresinizi girin.</div>'
          );
          return false;
        }
          ...
          .done(function (data) {
            if (data && data.ok) {
              if (data.status === "exists") {
                respBox.append(
                  '<div class="success">Bu e-posta zaten aboneliğe kayıtlı.</div>'
                );
              } else {
                respBox.append(
                  '<div class="success">Aboneliğiniz alındı. Teşekkürler!</div>'
                );
              }
              emailInput.val("");
            } else {
              respBox.append(
                '<div class="error">Bir hata oluştu. Lütfen tekrar deneyin.</div>'
              );
            }
          })
          .fail(function (xhr) {
            ...
            respBox.append('<div class="error">' + message + "</div>");
          })
          .always(function () {
            submitBtn.prop("disabled", false);
          });
  ...
  function getHashSelector(href) {
    ...
    return hash.charAt(0) === '#' ? hash : null;
  }
  ...
  function SmoothMenuScroll() {
    ...
    if (anchor.length) {
      anchor.children("a").bind("click", function (event) {
        ...
        var target = $(this);
        var href = getHashSelector(target.attr("href"));
        ...
        $("html, body")
          .stop()
          .animate({
            scrollTop: targetEl.offset().top - headerH + "px"
          },
            200,
            "easeInOutExpo"
          );
  ...
  function OnePageMenuScroll() {
    ...
    if (windscroll >= 117) {
      ...
      menuAnchor.each(function () {
        var sections = getHashSelector($(this).attr("href"));
        if (!sections) {
          return;
        }
  ...
  var _handleWindowLoad = function () {
  ...
  $(window).on("load", _handleWindowLoad);
